...
nc_white_final_norm = nc_white_final %>%
  mutate(total_searches = ifelse(county=="Greensboro", total_searches/115453*100,
                                 ifelse(county == "Charlotte", total_searches/347219*100,
                                        ifelse(county == "Durham", total_searches/115420*100,
                                               ifelse(county == "Raleigh", total_searches/245575*100,
                                                      ifelse(county == "Fayetteville", total_searches/72965*100,
                                                             total_searches/112053*100))))))
```
...
nc_black_final_norm = nc_black_final %>%
  mutate(total_searches = ifelse(county=="Greensboro", total_searches/128913*100,
                                 ifelse(county == "Charlotte", total_searches/307862*100,
                                        ifelse(county == "Durham", total_searches/102375*100,
                                               ifelse(county == "Raleigh", total_searches/131441*100,
                                                      ifelse(county == "Fayetteville", total_searches/88391*100,
                                                             total_searches/82855*100))))))
```
...
nc_white_final_norm %>%
  ...
  ggplot(aes(x = as.Date(ym(year_month)), y = total_searches)) +
    ...
    labs(title = "% Searches in Greensboro, NC and Winston-Salem, NC for White Subjects",
         y = "% of Searches/[Subject Race] Population") +
    scale_x_date(date_labels = "%m-%Y") +
    scale_color_manual(values=c("#6A7FDB", "#45CB85")) +
    theme_bw() +
    theme(plot.title = element_text(hjust = 0.5)) +
...
nc_white_final %>%
  ...
  ggplot(aes(x = as.Date(ym(year_month)), y = total_searches)) +
    ...
    xlab("") +
    labs(title = "Searches in Greensboro, NC and Winston-Salem, NC for White Subjects",
         y = "# Searches/[Subject Race] Population") +
    scale_x_date(date_labels = "%m-%Y") +
    scale_color_manual(values=c("#6A7FDB", "#45CB85")) +
    theme_bw() +
    theme(plot.title = element_text(hjust = 0.5)) +
